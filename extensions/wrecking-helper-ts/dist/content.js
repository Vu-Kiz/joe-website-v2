...
function showPrompt(current) {
  ...
  `;
    ...
    const input = document.getElementById("wrecking-helper-counter-input");
    if (!input) {
        return;
    }
    input.focus();
...
function dispatchSyntheticEnd(target) {
...
function tryInsertTextNow(text) {
...
function queueInsertForReadyPage(text) {
...
window.addEventListener("message", (event) => {
    ...
    chrome.runtime.sendMessage({ action: "websiteBridge", request: event.data }, (response) => {
        const payload = {
