...
class RmBrowserController extends Controller
{
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        $allEntities = [];
        $errors = [];
        $ttl = (int) env('RM_BROWSER_CACHE_SECONDS', 120);
        ...
        foreach ($selectedFactions as $factionUid) {
            ...
            $cacheKey = 'rm-browser:' . $factionUid . ':' . md5(serialize($filters));

            $result = $ttl > 0
                ? Cache::remember($cacheKey, $ttl, fn () => $this->rmBrowserService->fetchAllMaterials($accessToken, $factionUid, $label, $filters))
                : $this->rmBrowserService->fetchAllMaterials($accessToken, $factionUid, $label, $filters);
