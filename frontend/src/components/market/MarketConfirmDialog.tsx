...
const MarketConfirmDialog: React.FC<Props> = ({
  ...
  return (
    ...
    >
      <div
        className="flex flex-col gap-4 w-full max-w-115 p-4 rounded-[14px] border border-white/10 bg-[linear-gradient(180deg,rgba(26,28,34,0.98),rgba(19,20,25,0.98))] shadow-[0_20px_44px_rgba(0,0,0,0.5)]"
      ...
      >
        ...
        <div className="flex gap-[0.6rem] justify-end max-sm:flex-col">
          <button className={BTN_GHOST + " min-w-34 max-sm:w-full"} type="button" onClick={onClose}>Cancel</button>
          <button className={BTN + " min-w-34 max-sm:w-full"} type="button" onClick={onConfirm}>{confirmLabel}</button>
        </div>
      </div>
    </div>
  );
