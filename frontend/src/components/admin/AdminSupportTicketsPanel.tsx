      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[300px_1fr]">
          ...
          <div className="flex flex-col gap-2">
            {tickets.length === 0 && (
              <p className="small opacity-50">No tickets.</p>
            )}
            {tickets.map((ticket) => (
              <button
                key={ticket.id}
                type="button"
                onClick={() => { setSelected(ticket); setReply(""); setReplyError(null); }}
                className={`w-full text-left rounded-xl border px-4 py-3 transition-colors duration-100 ${
                  selected?.id === ticket.id
                    ? "border-[rgba(245,213,70,0.4)] bg-[rgba(245,213,70,0.06)]"
                    : "border-white/8 bg-surface hover:border-white/20 hover:bg-white/[0.05]"
                }`}
              >
          ...
          {selected ? (
            <div className="flex flex-col gap-4">
              <div className="flex items-start gap-3 flex-wrap">
                <div className="flex-1">
                  <p className="text-[0.75rem] opacity-50 m-0">#{selected.id} · {selected.tool_key}</p>
                  <h3 className="text-[1rem] font-semibold m-0 mb-2">{selected.title}</h3>
                  <div className="flex gap-2 flex-wrap">
            ...
            <p className="small opacity-40 text-center self-center">Select a ticket to view</p>
          )}
        </div>
      )}
    </div>
